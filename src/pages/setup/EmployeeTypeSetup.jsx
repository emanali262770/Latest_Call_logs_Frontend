...
export default function EmployeeTypeSetup() {
  ...
  const canCreate = hasPermission('employee-types.create');
  const canEdit = hasPermission('employee-types.update');
  const canDelete = hasPermission('employee-types.delete');
  ...
  const loadItems = useCallback(async (query = '') => {
    ...
    try {
      const response = await employeeTypeService.list(query);
  ...
  const handleSave = async () => {
    ...
    const nameError = required(trimmed, 'Employee Type');
    if (nameError) {
    ...
    try {
      if (editingItem) {
        const response = await employeeTypeService.update(editingItem.id, { name: trimmed, status });
        toast.success('Employee Type updated', response?.message || 'Employee Type updated successfully.');
      } else {
        const response = await employeeTypeService.create({ name: trimmed, status });
        toast.success('Employee Type created', response?.message || 'Employee Type created successfully.');
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await employeeTypeService.remove(deleteTarget.id);
      toast.success('Employee Type deleted', response?.message || 'Employee Type deleted successfully.');
  ...
  const modalTitle = editingItem ? 'Edit Employee Type' : 'Add Employee Type';
    ...
    <>
      <div className="space-y-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Employee Type Setup</h1>
            <p className="text-gray-500 mt-1">Manage employee types for classification and policy mapping.</p>
          </div>
          {canCreate && (
            <Button
            ...
            <div className="flex items-start justify-between gap-4 p-6 pb-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">Create or update a employee type record.</p>
                </div>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className="space-y-6 px-6 pb-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3 py-2 px-4 bg-brand-light/50 rounded-lg border border-brand/10">
                  ...
                  <span className="text-sm font-bold text-gray-900 tracking-tight">Employee Type Details</span>
                </div>
      ...
      <ConfirmDialog
        ...
        description={deleteTarget ? `Are you sure you want to delete ${deleteTarget.name || 'employee type'}?` : ''}
        confirmLabel="Delete"
