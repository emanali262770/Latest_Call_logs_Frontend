...
export default function DepartmentSetup() {
  ...
  const handleSave = async () => {
    ...
    const nameError = required(trimmed, 'Department');
    if (nameError) {
    ...
    try {
      if (editingItem) {
        const response = await departmentService.update(editingItem.id, { name: trimmed, status });
        toast.success('Department updated', response?.message || 'Department updated successfully.');
      } else {
        const response = await departmentService.create({ name: trimmed, status });
        toast.success('Department created', response?.message || 'Department created successfully.');
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await departmentService.remove(deleteTarget.id);
      toast.success('Department deleted', response?.message || 'Department deleted successfully.');
  ...
  const modalTitle = editingItem ? 'Edit Department' : 'Add Department';
    ...
    <>
      <div className="space-y-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Department Setup</h1>
            <p className="text-gray-500 mt-1">Manage departments available for employee assignment.</p>
          </div>
          {canCreate && (
            <Button
                  ...
                  {isLoading ? (
                    <tr>
                      <td colSpan={2} className="px-8 py-6 text-center">
                        <TableLoader label="Loading department records..." />
                      </td>
                    </tr>
                  ) : items.length === 0 ? (
                    <tr>
      ...
      <ConfirmDialog
        ...
        description={deleteTarget ? `Are you sure you want to delete ${deleteTarget.name || 'department'}?` : ''}
        confirmLabel="Delete"
