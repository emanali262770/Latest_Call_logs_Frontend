...
export default function CategoriesSetup() {
  ...
  const canCreate = hasPermission('INVENTORY.CATEGORY.CREATE');
  const canEdit = hasPermission('INVENTORY.CATEGORY.UPDATE');
  const canDelete = hasPermission('INVENTORY.CATEGORY.DELETE');
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        const response = await categoryService.update(editingItem.id, { name: trimmedName, status });
        toast.success('Category updated', response?.message || 'Category updated successfully.');
      } else {
        const response = await categoryService.create({ name: trimmedName, status });
        toast.success('Category created', response?.message || 'Category created successfully.');
      }
      closeModal();
  ...
  const handleDelete = async () => {
    ...
    setIsSaving(true);
    try {
      const response = await categoryService.remove(deleteTarget.id);
      toast.success('Category deleted', response?.message || 'Category deleted successfully.');
  ...
  return (
    <>
      <div className="space-y-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-gray-500">Manage stock categories for item classification.</p>
          </div>
          {canCreate && (
        ...
        <Card className="border-none p-0 shadow-xl shadow-gray-200/50">
          <div className="flex flex-col gap-4 border-b border-gray-50 p-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:w-96">
              ...
              <input type="text" placeholder="Search categories..." value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="w-full rounded-2xl border border-gray-100 bg-gray-50/50 py-3 pl-11 pr-4 text-sm placeholder:text-gray-400 transition-all focus:border-brand focus:outline-none focus:ring-4 focus:ring-brand/10" />
            </div>
              ...
              <table className="w-full min-w-[920px] border-separate border-spacing-0 text-left">
                <thead>
                  <tr className="bg-linear-to-r from-gray-50/80 via-gray-50/40 to-transparent">
                    ...
                    <th className="border-b border-gray-100/60 px-8 py-6 text-[10px] font-black uppercase tracking-[0.25em] text-gray-400">Category Name</th>
                ...
                <tbody className="divide-y divide-gray-50/50">
                  {isLoading ? (
                    <tr><td colSpan={4} className="px-8 py-6 text-center"><TableLoader label="Loading category records..." /></td></tr>
                  ) : items.length === 0 ? (
          ...
          <div className="relative z-10 w-full max-w-[430px] overflow-hidden rounded-3xl border-l-[6px] border-brand bg-white shadow-2xl shadow-brand/10">
            <div className="flex items-start justify-between gap-4 p-6 pb-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="text-lg font-bold tracking-tight text-gray-900">{editingItem ? 'Edit Category' : 'Add Category'}</h2>
                  <p className="mt-1 text-sm text-gray-500">Category name + active status</p>
                </div>
              </div>
            ...
            <div className="space-y-6 px-6 pb-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3 py-2 px-4 bg-brand-light/50 rounded-lg border border-brand/10">
                  ...
                  <span className="text-sm font-bold tracking-tight text-gray-900">Category Details</span>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Name <span className="text-red-500">*</span></label>
                  <input type="text" value={name} onChange={(event) => { setName(event.target.value); setValidationError(''); }} placeholder="Category name" className={`w-full rounded-xl border bg-white px-4 py-2.5 text-sm text-gray-900 transition-all focus:border-brand focus:outline-none focus:ring-4 focus:ring-brand/10 ${validationError ? 'border-rose-400' : 'border-gray-200'}`} />
      ...
      <ConfirmDialog isOpen={!!deleteTarget} title="Delete Category" description={deleteTarget ? `Are you sure you want to delete ${deleteTarget.name || 'category'}?` : ''} confirmLabel="Delete" onCancel={() => setDeleteTarget(null)} onConfirm={handleDelete} isLoading={isSaving} />
