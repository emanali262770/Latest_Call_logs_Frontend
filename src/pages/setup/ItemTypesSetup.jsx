...
const STORAGE_KEY = 'cms_setup_item_types';
...
export default function ItemTypesSetup() {
  ...
  const handleSave = () => {
    ...
    try {
      ...
      setItems(nextItems);
      saveItems(nextItems);
      toast.success(editingItem ? 'Item Type updated' : 'Item Type created', editingItem ? 'Item Type updated successfully.' : 'Item Type created successfully.');
      closeModal();
    } catch (errorMessage) {
  ...
  const handleDelete = () => {
    ...
    try {
      ...
      setItems(nextItems);
      saveItems(nextItems);
      toast.success('Item Type deleted', 'Item Type deleted successfully.');
  ...
  return (
    <>
                  ...
                  {isLoading ? (
                    <tr>
                      <td colSpan={3} className="px-8 py-6 text-center">
                        <TableLoader label="Loading item type records..." />
                      </td>
                    </tr>
      ...
      <ConfirmDialog
        ...
        description={deleteTarget ? `Are you sure you want to delete ${deleteTarget.name || 'item type'}?` : ''}
        confirmLabel="Delete"
