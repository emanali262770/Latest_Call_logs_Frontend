...
export default function DutyShiftSetup() {
  ...
  const canCreate = hasPermission('duty-shifts.create');
  const canEdit = hasPermission('duty-shifts.update');
  const canDelete = hasPermission('duty-shifts.delete');
  ...
  const loadItems = useCallback(async (query = '') => {
    ...
    try {
      const response = await dutyShiftService.list(query);
  ...
  const handleSave = async () => {
    ...
    const nameError = required(trimmed, 'Duty Shift');
    if (nameError) {
    ...
    try {
      if (editingItem) {
        const response = await dutyShiftService.update(editingItem.id, { name: trimmed, status });
        toast.success('Duty Shift updated', response?.message || 'Duty Shift updated successfully.');
      } else {
        const response = await dutyShiftService.create({ name: trimmed, status });
        toast.success('Duty Shift created', response?.message || 'Duty Shift created successfully.');
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await dutyShiftService.remove(deleteTarget.id);
      toast.success('Duty Shift deleted', response?.message || 'Duty Shift deleted successfully.');
  ...
  const modalTitle = editingItem ? 'Edit Duty Shift' : 'Add Duty Shift';
    ...
    <>
      <div className="space-y-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Duty Shift Setup</h1>
            <p className="text-gray-500 mt-1">Manage available duty shifts for roster assignment.</p>
          </div>
          {canCreate && (
            <Button
            ...
            <div className="flex items-start justify-between gap-4 p-6 pb-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">Create or update a duty shift record.</p>
                </div>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className="space-y-6 px-6 pb-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3 py-2 px-4 bg-brand-light/50 rounded-lg border border-brand/10">
                  ...
                  <span className="text-sm font-bold text-gray-900 tracking-tight">Duty Shift Details</span>
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">
                    Duty Shift <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={value}
      ...
      <ConfirmDialog
        ...
        description={deleteTarget ? `Are you sure you want to delete ${deleteTarget.name || 'duty shift'}?` : ''}
        confirmLabel="Delete"
