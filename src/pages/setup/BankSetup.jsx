...
export default function BankSetup() {
  ...
  const canEdit = hasPermission('banks.update');
  ...
  const loadItems = useCallback(async (query = '') => {
    ...
    try {
      const response = await bankService.list(query);
  ...
  const handleSave = async () => {
    ...
    const nameError = required(trimmed, 'Bank');
    if (nameError) {
    ...
    try {
      if (editingItem) {
        const response = await bankService.update(editingItem.id, { name: trimmed, status });
        toast.success('Bank updated', response?.message || 'Bank updated successfully.');
      } else {
        const response = await bankService.create({ name: trimmed, status });
        toast.success('Bank created', response?.message || 'Bank created successfully.');
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await bankService.remove(deleteTarget.id);
      toast.success('Bank deleted', response?.message || 'Bank deleted successfully.');
  ...
  const modalTitle = editingItem ? 'Edit Bank' : 'Add Bank';
    ...
    <>
      <div className="space-y-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Bank Setup</h1>
            <p className="text-gray-500 mt-1">Manage banks available for salary and reimbursement setup.</p>
          </div>
          {canCreate && (
            <Button
            ...
            <div className="flex items-start justify-between gap-4 p-6 pb-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">Create or update a bank record.</p>
                </div>
              </div>
              <button
                onClick={closeModal}
            ...
            <div className="space-y-6 px-6 pb-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3 py-2 px-4 bg-brand-light/50 rounded-lg border border-brand/10">
                  ...
                  <span className="text-sm font-bold text-gray-900 tracking-tight">Bank Details</span>
                </div>
      ...
      <ConfirmDialog
        isOpen={!!deleteTarget}
        title="Delete Bank"
        description={deleteTarget ? `Are you sure you want to delete ${deleteTarget.name || 'bank'}?` : ''}
        confirmLabel="Delete"
